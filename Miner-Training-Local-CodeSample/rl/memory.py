...
class Memory(object):
    ...
    def get_recent_stateCNN(self, current_observation):
        """Return list of last observations
        ...
        # from different episodes. We ensure that an experience never spans multiple episodes.
        # This is probably not that important in practice but it seems cleaner.
        state = [current_observation[:, :, 0], current_observation[:, :, 1]]
        idx = len(self.recent_observations) - 1
        for offset in range(0, self.window_length - 1):
            current_idx = idx - offset
            current_terminal = self.recent_terminals[current_idx - 1] if current_idx - 1 >= 0 else False
            if current_idx < 0 or (not self.ignore_episode_boundaries and current_terminal):
                ...
                # Otherwise we would leak into a different episode.
                break
            state.insert(0, self.recent_observations[current_idx][:, :, 1])
            state.insert(0, self.recent_observations[current_idx][:, :, 0])
        while len(state) < self.window_length*2:
            state.insert(0, zeroed_observation(state[0]))
        return state
...
class SequentialMemory(Memory):
    ...
    def sampleCNN(self, batch_size, batch_idxs=None):
        """Return a randomized batch of experiences
        ...
        for idx in batch_idxs:
            ...
            # from different episodes. We ensure that an experience never spans multiple episodes.
            # This is probably not that important in practice but it seems cleaner.
            state0 = [self.observations[idx - 1][:, :, 0], self.observations[idx - 1][:, :, 1]]
            for offset in range(0, self.window_length - 1):
                current_idx = idx - 2 - offset
                assert current_idx >= 1
                current_terminal = self.terminals[current_idx - 1]
                if current_terminal and not self.ignore_episode_boundaries:
                    ...
                    # Otherwise we would leak into a different episode.
                    break
                state0.insert(0, self.observations[current_idx][:, :, 1])
                state0.insert(0, self.observations[current_idx][:, :, 0])
            while len(state0) < self.window_length * 2:
                state0.insert(0, zeroed_observation(state0[0]))
            action = self.actions[idx - 1]
            reward = self.rewards[idx - 1]
            terminal1 = self.terminals[idx - 1]
            ...
            # to the right. Again, we need to be careful to not include an observation from the next
            # episode if the last state is terminal.
            state1 = [np.copy(x) for x in state0[2:]]
            state1.append(self.observations[idx][:, :, 1])
            state1.append(self.observations[idx][:, :, 0])

            assert len(state0) == self.window_length * 2
            assert len(state1) == len(state0)
            experiences.append(Experience(state0=state0, action=action, reward=reward,
                                          state1=state1, terminal1=terminal1))
        assert len(experiences) == batch_size
